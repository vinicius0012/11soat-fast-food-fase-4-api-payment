...
### Autenticação JWT
O serviço utiliza JWT (JSON Web Tokens) para autenticação de endpoints administrativos.

### Validação de Webhooks
Os webhooks do Mercado Pago são validados através de:
...
### Métricas
Integração com SonarCloud para:
- Qualidade de código
- Cobertura de testes
- Vulnerabilidades
- Code smells
...
Este microserviço pode ser integrado com:

### API de Pedidos
- Recebe `orderId` ao criar pagamento
- Notifica mudanças de status via callback URL

### API de Notificações
- Envia eventos de pagamento aprovado/rejeitado
...
const paymentResponse = await fetch('http://payment-service:1337/payment/create', {
  method: 'POST',
  ...
  body: JSON.stringify({
    ...
    orderId: order.id,
    callbackUrl: 'http://order-service:3000/orders/payment-callback',
    expirationMinutes: 30
  })
});

const { transactionId, qrCode } = await paymentResponse.json();
```
...
Este projeto é parte do trabalho de conclusão do curso de Pós-Graduação em Software Architecture da FIAP - Turma 11SOAT.

## 👥 Contribuidores

Desenvolvido pela equipe do projeto Fast Food Totem - Fase 4.
